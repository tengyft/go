...
func (b *Buffer) grow(n int) int {
	...
	if m == 0 && b.off != 0 {
		// 没有未读的字节，并且读指标位置不是0就恢复使用底层的存储空间
		// 在此b.off != 0检测的意义是：防止Buffer重新执行b.Truncate(0)，以致于做了额外的功。
		b.Truncate(0)
	}
	if len(b.buf)+n > cap(b.buf) {
		// Buffer底层存储空间不足以再读入n个字节
		var buf []byte
		if b.buf == nil && n <= len(b.bootstrap) {
			// Buffer首次读入n个字节，n <= len(b.bootstrap)时
			buf = b.bootstrap[0:]
			...
			// 如果新读入n个字节后，未读的字节数(m+n)不足Buffer底层存储空间总大小的二分之一，就直接使用现在的底层存储空间。
			// 至于说为什么要求是总大小的二分之一而不是小于总大小，那是为了减小拷贝时间。
			copy(b.buf[:], b.buf[b.off:]) // 将未读的字节拷贝到Buffer底层存储空间起始处。
			buf = b.buf[:m]
		} else {
			// not enough space anywhere
			// 新读入n个字节后，未读的字节数(m+n)已经大于Buffer底层存储空间总大小的二分之一，则重新分配一个大的底层存储空间。
	...
	b.buf = b.buf[0 : b.off+m+n] // 保证可以再读入了n个字节。
	return b.off + m             // 返回新的Buffer写位置
}
...
// returns an error.
// UnreadByte会unread最近一次读操作读取的一个字节。
// 如果最近一次操作是write类型的，则UnreadByte会返回错误。
